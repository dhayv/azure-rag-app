...
# Load .env from rag-app directory (parent of service)
...
def chunk_preserve_code(text: str, max_len=1600, overlap=200) -> List[str]:
    parts: List[str] = []
    # split by fenced code blocks (keep them)
...
def embed(texts: List[str]) -> List[List[float]]:
    # Azure OpenAI: embeddings.create with deployment name
...
def to_docs(path: str) -> List[Dict[str, Any]]:
    ...
    # strip header marker if present
...
def upload_batch(docs: List[Dict[str, Any]]):
    # azure-search-documents does automatic batching up to ~1000 docs or payload size
...
def main():
    ...
    for p in tqdm(paths, desc="Ingesting"):
        docs = to_docs(p)
        # Upload in small batches if doc is very large; usually chunking keeps it small
        upload_batch(docs)
        total_docs += len(docs)
